...
const NotificationView = () => {
  ...
  return (
    <div className="bg-white py-4 pl-2 w-[330px] h-[calc(100vh-70px)] absolute top-14 right-0 login_box rounded-md z-40 ">
      ...
      <div className='flex gap-1 my-2 pl-2'>
        <div onClick={() => setPick("all")}>
          <button className={`${pick === "all" ? cssPick : cssUnPick} font-semibold px-3 py-1  rounded-2xl`}>
            <span >All</span>
          </button>
        </div>
        <div onClick={() => setPick("unread")}>
          <button className={`${pick === "unread" ? cssPick : cssUnPick} font-semibold px-3 py-1  rounded-2xl`}>
            <span >Unread</span>
          </button>
        </div>
      </div>
